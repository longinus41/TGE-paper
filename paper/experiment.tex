...
In the first experiment, we test XXX on classification with the label set. Note that one account is supposed to have single identity instead of multiple identities. Reason for this is that, people who take on many identities, tend to hold multiple accounts usually and each account is related to one identity. 
...
Results are summarized in TABLE~\ref{table:overall_results}. Our methods (with asymmetric proximity only and with asymmetric proximity and time-density both) outperform than others in average $F_1$ score. Note that rGCN achieves the highest average precision but a low average recall. Especially, rGCN has the lowest recall of category of phish and hack which means that many normal accounts are inferred as threatening in rGCN model.
