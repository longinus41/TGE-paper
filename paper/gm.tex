...
\paragraph{Account representations} To encode the overall structural and additional information of accounts, we use a feature matrix $X \in \mathbb{R}^{N \times d}$. The matrix $X$ includes the following $d$ pre-defined vectors.
...
To investigate the multi-relations between accounts in Ethereum transaction graph, we divide the raw transaction graph into several sub-graphs and capture the relations of graph neighboring nodes using adjacent matrices.
